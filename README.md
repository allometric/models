...
This repository stores allometric models as a language-agnostic **YAML truth
source** for the [`allometric`](https://github.com/allometric) ecosystem.
Because there are thousands of allometric models in existence, a separate
repository is needed to store and track their development. We treat `models`
as a data repository, not a package.

- **YAML is the source of truth.** Each publication is one file under
  `publications/` (grouped by author range: `a_e`, `f_j`, `k_o`, `p_t`,
  `u_z`). Files are reviewed, git-diffable, and contain every model from the
  publication, including parameterized sets of models.
- **The `orc` tooling validates and indexes the corpus.** [`orc`](https://github.com/allometric/orc)
  ingests the YAML, validates each file against a declared schema, derives a
  stable 8-character content hash for every model, and emits a flat registry
  for downstream compilation (Arrow/Parquet).
- **Everything is content-addressed.** A model's id is a hash of its own YAML
  content, so editing a model changes its id and identical models across
  publications collapse to the same id.

Users interested in *contributing* models to `allometric` are in the right
location.

Users interested in *using* allometric models for analysis should refer to the
`allometric` package.
...
Motivated users can learn how to author model YAML files and submit pull
requests. Users comfortable with YAML and git can handle this task.
